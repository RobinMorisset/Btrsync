...
\subsubsection{Variant 3:} $p_i = 2^{ut_i}$. 
Using Algorithm~TODO for the computation of $c_i$, this variant offers substantial constant-factor accelerations compared to previous variants, because it removes the need of all modulo operations and of {\sc crt} re-combination when multiple $p_i$ are needed, since $C_i$ is just the binary concatenation of $c_i$ and $C'_{i-1}$ !
...
The non-overwhelming (but nonetheless important) complexities of the computations of $(c,c')$ and of the factorizations can be even reduced to $\Oapp(\frac{n}{t_i} \mu(u t_i))$ and $\Oapp(\frac{n}{T_i} \mu(u T_i)$ using product trees. 
