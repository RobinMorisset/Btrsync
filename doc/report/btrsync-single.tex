...
\begin{itemize}
\item $t_1 = t_2 = \dots = t$, in which case $\lambda = \lceil T/t \rceil$;
\item $t_k = 2^k t'$, in which case $ \lambda = \lceil \log_2(T / t') \rceil $.
\end{itemize}
...
\[  \sum_{k=1}^\lambda \log c_k + \log b + \log |\SetD'|
 \,\leq\, \sum_{k=1}^\lambda (ut_k+1) + \frac{1}{2} (uT_\lambda) + u T
 \,\leq\, u (T_\lambda + \lambda) + \frac{1}{2} u T_\lambda + u T
 \,\leq\, \frac{5}{2} u T_\lambda + u \lambda, \]
where $\lambda$ is the maximum number of rounds needed.

Here are the complexity in the two cases we are interested in:
\begin{itemize}
\item if $t_1 = \dots = t$, $\lambda = \lceil T/t \rceil$, $T_\lambda = \lambda t < T+t$ and the transmission complexity is at most $\frac{5}{2} u (T+t) + \lceil T/t \rceil = O(uT)$;
\item if $t_k = 2^k t'$, $\lambda = \lceil \log(T/t') \rceil$, $T_\lambda < 2T$ and the transmission complexity is at most $\frac{5}{2} 2uT + \lceil \log(T/t') \rceil = O(ut)$.
\end{itemize}
...
By opposition to section \ref{sec:basic} we do not have a fixed bound for $a$ and $b$ anymore; Equation \eqref{eqab} only provides a bound for the \textit{product} $a b$. Therefore, we define a sequence of $t+1$ couples of bounds:
...
If failures never occur, this variant will roughly halve the quantity of transmitted bits with respect to section \ref{sec:basic}.\smallskip
...
\label{shortu}
What happens if we brutally shorten the hash size $u$ in the \df protocol?\smallskip
...
In this section, after showing how to deal with these collisions and adapting \df subsequently, we analyse the advantages of this size reduction and show how to select $u$.
...
We now combine both optimizations and assume that $\ell$ laundry rounds are necessary for completing some given reconciliation task using a half-sized $p$. By opposition to Section~\ref{sec:basic}, confirming correct protocol termination is now non-trivial.\smallskip
...
If $v$ of $\ell+v$ rounds fail, Oscar needs to solve a problem called \textit{Chinese Remaindering With Errors}~\cite{phong}:\smallskip
...
\label{sec:hashprime}
Hashing into primes is frequently needed in cryptography. A recommended implementation of $\mbox{{\tt HashPrime}}(F)$ is given in Algorithm \ref{alg:primes}. If $u$ is large enough (\eg $160$) one might sacrifice uniformity to avoid repeated file hashings by defining $\mbox{{\tt HashPrime}}(F)=\mbox{{\tt NextPrime}}(\mbox{{\tt Hash}}(F))$. 
...
The same technique applies to factorization\footnote{We explain the process with $a$, this is applicable \textit{ne variatur} to $b$ as well.}, but with a slight \textit{caveat}.\smallskip
