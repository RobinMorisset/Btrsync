...
As proven in Section~\ref{sec:hashprime}, the complexity of a naive algorithm to perform $\tt HashPrime$ is $u^2 \mu(u)$.
...
Hashing into primes is frequently needed in cryptography. 
%A recommended implementation of $\HashPrime(F)$ is given in Algorithm \ref{alg:primes}
A way to implement such hash function $\HashPrime$ is to use Algorithm~\ref{alg:primes}, where $\Hash'$ is a a hash function with a $u$-bit output.
If $\Hash'$ can be considered as a random function, the output of $\HashPrime$ is also a uniformly random $u$-bit prime number.
...
Let us very roughly show why this second algorithm should perform about $\frac{\varphi(\alpha)}{\alpha}$ times fewer primality test, where $\varphi$ is the Euler's totient function.
...
Therefore, the algorithms needs to do about $\frac{2^u}{u \varphi(\alpha)} / \frac{2^u}{\alpha} = \frac{\alpha}{\varphi(\alpha)} u$ primality tests before finding a prime number, whereas the naive algorithm has to do about $u$ tests.
Hence this algorithm is about $\frac{\alpha}{\varphi(\alpha)}$ times faster.

One main drawback of this algorithm is that, even if $\Hash'$ can be modeled as a random function, the output of $\HashPrime$ is no more a uniform random $u$-bit prime number.
...
The same technique applies to factorization\footnote{We explain the process with $a$, this is applicable \textit{ne variatur} to $b$ as well.}.
...
This increases the transmission complexity slightly, but drastically reduces the computational complexity (see Table~\ref{tab:workload}).
