...
TODO (Fabrice): this may not be the good place !! and it is important to ntoe that we go further simple set reconciliation
...
TODO (Fabrice): another point is that we use a global hash to check termination, whereas MTZ uses another method which is maybe less optimal ! But this is quite trivial...

TODO: introduce $\Oapp$

Secondly, the algorithm, which we call PART-MTE, uses MTZ as a blackbox, and which performs set reconciliation using at least about $24e$ times more bandwith (with a partition in $2$ parts), but substantially improves the computational complexity of MTZ.
...
With the optimization of Section~\ref{alg:div-prod-tree}, it costs $\Oapp(\frac{n}{T_k} \mu(nuT_k))$.
But with MTZ, we can also do a direct factorization of $a$ and $b$ in time $O(T_k^3 \mu(u))$.

TODO: say that we can drop this complexity to $\Oapp(T_k^\omega \mu(u))$, with $\omega$ the exponent of matrix mult. ?????
 \url{http://www.google.fr/url?sa=t&rct=j&q=&esrc=s&source=web&cd=3&cad=rja&ved=0CDcQFjAC&url=http%3A%2F%2Freference.kfupm.edu.sa%2Fcontent%2Ff%2Fa%2Ffactoring_polynomials_over_finite_fields_120357.pdf&ei=M02ZUJWtJYGW0QWe7oDYCw&usg=AFQjCNGirX3kl2V4zJsaA9cZ0nK-lKzjDA}

However, this can only decrease the overall asymptotic computational complexity of MTZ, since the computation of the redundacies is alrea TODO...
