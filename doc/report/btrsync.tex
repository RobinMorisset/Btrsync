...
\begin{itemize}
  ...
  \item We explain in Section~\ref{hashing} how \df can be extended with hash
    functions to perform \emph{file reconciliation}, i.e., reconcile sets of files which do not have a fixed size. We
    ...
    points of this analysis (TODO: lesquels?! --- Fabrice: tous  en fait, au problème du hashing vers les nbs premiers près, on utilise \df en blackbox !) apply no matter the choice of the underlying set
  ...
  \item We spell out in Section~\ref{files} how the previous construction can be
    extended to perform \emph{file synchronization}, taking into account the location
...
if $\Hash(S') \neq H$ go back to phase 1 (event $\bot$) & & \\
\bottomrule
\end{tabular}\vspace{-0.25cm} % why the hell do we need that?!
...
Assuming that there is no event $\bot$ (since these events happen with negligible probability), the transmission complexity of \df is as follows:
...
This section describes a proposed constant-factor improvement to the transmission complexity of \df which works by reducing the size of the prime number $p$. This allows \df to transfer more information at the expense of a higher failure rate when performing rational number reconstruction.
...
The difference between the proposed improvement and the original protocol is that the prime number $p$ that we generate should be about twice smaller than the $p$ recommended in Section~\ref{sec:basic}, namely:
\begin{equation}
\label{eqnewp}
2^{ut-1}\leq p < 2^{ut}
\end{equation}
...
The drawback is that, unlike in Section~\ref{sec:basic}, we have no guarantee that such an $(a,b)$ is unique. Namely, we could in theory stumble over an $(a',b')\neq (a,b)$ satisfying (\ref{eqab}) for some index $i' \neq i$. We conjecture that such failures happen with negligible probability (that we do not try to estimate here) when $u$ is large enough, so that the expected number of bits to transfer with this variant is lower.
...
In this section, we show how hashing makes it possible to use \df (or any set reconciliation protocol) to perform \emph{file reconciliation}.
We then show methods specific to \df that can reduce the size of hashes, and thus improve the transmission complexity by constant factors.
...
Then, when the protocol succeeds, Neil can send to Oscar the actual content of the files, corresponding to hashes in $\SetD'$, \ie, the files Oscar is missing.
...
\end{align*}
where $\HashPrime$ is a collision-resistant hash function to prime numbers, so that the $h_i$ and $h'_i$ identify their corresponding file in $F_i$ or $F'_i$ without ambiguity.
...
% je commente cette footnote, parce que c'est dans sec:hashprime qu'il faut expliquer ça
% \footnote{This is the case if we use our first proposed Algorithm~\ref{alg:primes} with a classical hash function which can be modeled as a random function.
%We suppose this is true for well-known hash functions such as SHA-256. (TODO ?)}
...
Thus, it suffices to replace the hash of files $h_i = \HashPrime(F_i)$ (or $h'_i = \HashPrime(F'_i)$) by $\hbar_{i,\ell}=\HashPrime(\ell|F_i)$ (or $\hbar_{i,\ell} = \HashPrime(\ell|F'_i)$ to quickly filter-out file differences by repeating the \df protocol for $\ell=1,2,\ldots$. 
At each iteration, the parties will detect files which are not in common (\ie, files $F_i$ or $F'_i$ whose hash $\hbar_{i,\ell}$ or $\hbar'_{i,\ell}$ does not collide), reconciliate these differences (\ie, remove the relevant files from $\Files$ and $\Files'$) and ``launder'' again the remaining files $\Files$ and $\Files'$.

We still need a condition to stop ``laundering'' files, \ie, a condition to ensure that there are no more collisions which hide remaining differences.
Before showing this condition, let us first analyse precisely the three kinds of collisions which can appear over an execution $\ell$ of \df:
...
These collisions can be easily detected by Oscar or Neil, at the end of the execution $\ell$ of the \df protocol. 
TODO(antoine) je ne comprends absolument pas ces deux phrases. This will prevent them from knowing easily to which file $F$ corresponds a hash not in common $h$.
...
\item Collisions between files not in common, \ie, files in $\Files \Delta \Files'$.
  TODO(antoine) à reformuler: These collisions obliviate a real difference between $\Files$ and $\Files'$. If no mechanism is added, these collisions can never be detected.
\end{enumerate}

TODO(antoine): à clarifier.
We propose the following method to check termination.
Before anything else is sent, Neil needs to send a global hash $H'$ on a collision-resistant hash of files: $H' = \Hash((\Hash(F'_1),\dots,\Hash(F'_{n'})))$.
...
We notice that, as soon as a file $F_i$ or $F'_i$ in $\Files \Delta \Files'$ does not collide with another file in one execution of \df, then this difference between $\Files$ and $\Files'$ will be detected by our new protocol.
Therefore, in this naive analysis, we just compute the probability that a file keeps colliding for $\lambda$ executions, to obtain an upper bound on the probability that at least one difference between $\Files$ and $\Files'$ will not be detected after $\lambda$ executions (\ie, at least one false positive survives $\lambda$ executions).
...
To understand why the probability that a stubborn file continues to collide decreases exponentially with the number of iterations $\lambda$, assume that $\eta$ remains invariant between iterations and define the following random variables:
\[
\begin{array}{rcl}
X^{\ell}_i & = &
\left\{
...
For the previously considered instance of $\eta=10^6$ and $u=32$, we get $\epsilon_2 \le 5.43\%$ and $\epsilon_3 \le 2 \cdot 10^{-3}\%$.
In particular, this means that with probability more than $1-\epsilon_2 \approx 95\%$, two executions of \df will suffice.
...
\label{comp}
In this section, we determine the computational complexity of \df. We first describe the complexity using a straightforward implementation, and present how the complexity can be improved using four independent optimizations. A summary of all costs can be found in Table~\ref{tab:workload}.
...
$u^2 \mu(u)$ (see Section~\ref{sec:hashprime} for details).
...
Hashing into primes is frequently needed in cryptography, independently from \df. In this section, we present a naive way to do so, and present an optimiation which sacrifices uniformity to achieve better performance.

\subsubsection{Naive Algorithm.}
\label{sec:basic}

%A recommended implementation of $\HashPrime(F)$ is given in Algorithm \ref{alg:primes}
A way to implement such hash function $\HashPrime$ is to use Algorithm~\ref{alg:primes}, where $\Hash'$ is a hash function with a $u$-bit output.
...
Let us sketch a proof of the fact that this second algorithm performs about $\frac{\varphi(\alpha)}{\alpha}$ times fewer primality tests, where $\varphi$ is the Euler's totient function.
...
Therefore, the algorithm needs to do about $\frac{2^u}{u \varphi(\alpha)} / \frac{2^u}{\alpha} = \frac{\alpha}{\varphi(\alpha)} u$ primality tests before finding a prime number, which improves over the $u$ tests required by the naive algorithm on average.
...
This slightly increases the number of collisions of $\HashPrime$ and $u$ has to be increased subsequently. (TODO ?)
...
Taking the $p_k$'s to be $ut_k$-bit primes is not very practical, because generating a big prime number is slow, and storing a list of such primes require to be able to bound the number of rounds, and to fix all the parameters ($u$ and $t_1,t_2,\dots$).\comm{``bound the number of rounds''? Oscar et Neil ne peuvent pas se mettre d'accord sur une seed et en tirer une séquence infinie de premiers à utiliser?}
In this section, we study alternate choices for the $p_k$ which are easier to generate and speed up the computation by constant factors.

Let $\mbox{{\tt Prime}}[i]$ denote the $i$-th prime, with $\mbox{{\tt Prime}}[1]=2$. Besides conditions on size, the \textit{only} property required from $p$ is to be co-prime with all the $h_i$'s and all the $h'_i$'s. We can hence consider the following variants:
...
$\mbox{{\tt Prime}}[k]^{r_k}$.\comm{Je ne comprends pas: on ne \emph{choisit} pas les $h_k$, si ?}
...
Therefore, the cost of these steps drops to $\Oapp(n u)$ with {\sc fft}~\cite{schonhage1971schnelle}. 
To simplify the presentation, we suppose there is only one round, write $p=p_1$, $t=t_1=T_1$, and assume that $t=2^\tau$ is a power of two dividing $n$.
...
As seen in Section~\ref{sec:complete-df}, instead of using $t_1 = t_2 = \dots = t$, we can use $t_k = 2^k t'$, and double $t_k$ at each iteration.
...
Both  & computation of $h_i$ and $h'_i$ 
      & $n u^2 \mu(u)$ 
      & $\frac{\alpha}{\phi(\alpha)} n u^2 \mu(u)$
...
\bottomrule
  \end{tabularx}
  \begin{tablenotes}
    \item[a] using the basic algo of Section~\ref{sec:basic}, and taking $t_1=t_2=\dots=t$.
    \item[b] using all the optimizations of Sections~\ref{sec:hashprime}, \ref{sec:choicep} ($p_k = 2^{ut_k}$), \ref{sec:algo-opt-prod-trees} and~\ref{sec:doubling}. In addition, choosing $p_k = 2^{ut_k}$ provides substantial constant factor accelerations which cannot be seen on the asymptotic complexities given in this table.
    \item[c] only for $p_i$ prime (or variant~1 or~2 in Section~\ref{sec:choicep});
    \item[d] only for $p_i = 2^{ut_i}$. TODO: Antoine: dans ce cas, pourquoi ça apparaît dans la colonne basic ? Fabrice: ???????
...
described in Section~\ref{sec:hashprime}. Besides, we did not implement the
proposed improvement in transmission complexity
(Section~\ref{sec:probabilistic}).
...
hasn't changed and the underlying call to \rsync can proceed in a clever manner.
...
\item S'assurer qu'on explique explicitement comment/pourquoi on passe de set
  ...
  unidirectionnelle).
\item Fabrice: Comparaison théorique et bien insister sur ce que l'on apporte réellement.
\end{itemize}
