...
After introducing the problem and the notations, we first present a basic version of \df which assumes that the number of differences between the two sets is bounded by some constant.
We then show how to extend this protocol to deal with any number of differences.
...
\begin{theorem}
\label{theo}
Let $a,b \in {\mathbb Z}$ be two co-prime integers such that $0 \leq a \leq A$ and $0<b \leq B$. Let $p>2AB$ be a prime and $s=a b^{-1} \bmod p$. Then $a,b$ are uniquely defined given $s$ and $p$, and can be recovered from $A,B,s,p$ in polynomial time.
\end{theorem}
...
This check has the advantage to be very fast, and it does not require Neil to send any data to Oscar.
...
Whenever Oscar's $\Set'$ is incorrect because the assumption $T < t$, this check will fail unless we found a collision in $\Hash$. Hence, we can bound the probability of error of the protocol by $2^n$. In what follows, we assume that our choice of $n$ is sufficient to ensure that the probability of error is negliglible: this only adds an additive constant to the transmission complexity of the protocol.

\subsubsection{Complete \df Protocol}
To extend the protocol to an arbitrary $T$, Oscar and Neil agree on an infinite set of primes $p_1,p_2,\ldots$ As long as the protocol fails (\ie, yields a bad reconciliation), Neil and Oscar redo the protocol with a new $p_\ell$ and Neil will keep accumulating information about the difference between $\Set$ and $\Set'$ as shown in Appendix \ref{sec:extended}. 
