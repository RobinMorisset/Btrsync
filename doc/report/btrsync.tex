...
Note that this section is not concerned about the file reconciliation and file
synchronization problems.

\paragraph{The MTZ algorithm.}
An existing set reconciliation algorithm achieving linear transmission
complexity is the one by Minsky, Trachtenberg and Zippel in~\cite{Mins1} (which
we call MTZ). In contrast with MTZ which is based on polynomials, our
independently found \df algorithm is based on number theory.
\df and MTZ have roughly the same transmission complexity, if we omit the fact
...
\paragraph{Complexity of MTZ and \df.}
In this section, we only consider the complete \df protocol for set reconciliation: we therefore do not take the complexity of the computation of $h_i$ and $h'_i$ into account. Let us compare the computational complexities of \df and MTZ.
...
We can thus easily see that all the optimizations (except, of course, the ones of Section~\ref{sec:choicep}) would also be applicable to MTZ.
...
Concerning the PART-MTZ and PART-\df protocols, we can remark that, their overall complexity is the same (up to logarithmic factors) and is roughly $\Oapp(n \mu(u))$.
We can notice that in~\cite{Mins1,PSRec}, the authors do not consider the computation of the redundancies in the overall complexity, because they consider an ``incremental'' model where sets $\Set$ and $\Set'$ are often updated (by adding or removing a value in $\Set$ or $\Set'$) and often synchronised.
...
There seem to be no obvious way to extend this optimization for the ``incremental'' setting to \df.
...
Other set reconciliation algorithms such as the ones from~\cite{Whats} and~\cite{byers} have been benchmarked against MTZ~\cite{Whats} and found to have better running times but worse bandwidth requirements. Our main focus is on transmission complexity, but it would be an interesting study to compare PART-\df and PART-MTZ with such algorithms.
