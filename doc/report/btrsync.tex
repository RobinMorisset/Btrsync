...
\label{comp}
In this section, we are interested in computing the computational complexity of our protocol. To simplify the analysis, we assume that there are no collisions.
...
\comm{Refaire ce tableau: ajouter une ligne "total", garder uniquement la pire colonne et la meilleure colonne, ajouter une colonne "section" avec une reference vers la section qui explique.}
...
re-transferring the contents of the moved files (which is something that the
most used synchronization tool, namely \texttt{rsync}, doesn't do).
