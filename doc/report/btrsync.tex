...
Each $H_j$ can be computed in $\Oapp(\mu(u t))$ using the standard product tree method described in Algorithm \ref{alg:prod-tree} (for $j=0$) and illustrated in Figure \ref{fig:prod-tree}. And all these $\frac{n}{t}$ products can be computed in $\Oapp(\frac{n}{t} \mu(u t))$. Then, one can compute $c$ by multiplying the $H_j$ modulo $p$, which costs $\Oapp(\frac{n}{t} \mu(u t))$.
...
\Function{prodTree}{$i$,$\vstart$,$\vend$}
  ...
  \Else
    ...
    \State $\pi_i \gets $\Call{prodTree}{$2i$,$\vstart$,$\vmid$}
    \State $\pi_{i+1} \gets $\Call{prodTree}{$2i+1$,$\vstart$,$\vmid$}
...
\comm{Fabrice il y a un $\times$ orphelin dans l'algo à l'étape 11, peux tu STP clarifier ?}
...
After computing the tree product we can compute the residues of $a$ modulo $H_j$. Then we can compute the residues of $a \mod H_j$ modulo the two children of $H_j$ in the product tree (namely: $\prod_{i=j t}^{j t + t/2 - 1} h_i$ and $\prod_{i=j t}^{j t + t/2 - 1} h_i$), and so on. Intuitively, we descend the product tree doing modulo reduction. At the end ({\sl i.e.}, as we reach the leaves), we obtain the residues of $a$ modulo each of the $h_i$. This is described in Algorithm \ref{fig:div-prod-tree} and illustrated in Figure \ref{fig:div-prod-tree} (for $j=1$). Complexity is $\Oapp(\mu(u t))$ for each $j$, summing-up to a total complexity of $\Oapp(\frac{n}{t} \Oapp(\mu(u t)))$.

\comm{Fabrice, peux tu stp verifier tous ces $\Oapp$s? je crains qu'en editant j'ai fais des erreurs}
...
\subsubsection{Variant 2:} Progressively work modulo a long power of two. This variant, is probably the most efficient of all but somewhat complex to explain. We hence describe it in detail in Appendix \ref{powtwo}.
...
All executables and source codes can be downloaded from \cite{Robin}.\smallskip
...
~~{\bf Directory}              ~~&~~{\bf Description}\\\hline\hline
~~{\tt synthetic}              ~~&~~A directory containing 1000 very small files containing~~\\
~~                             ~~&~~the numbers $1,2,\ldots,1000$. \\\hline
~~{\tt synthetic\_shuffled}    ~~&~~{\tt synthetic} with:\\
                             ~~& ~~~~~10 deleted files\\
                             ~~& ~~~~~10 renamed files \\
                             ~~& ~~~~~10 modified files \\\hline
~~{\tt source}                 ~~& ~~A snapshot of \btrsync's own source tree \\\hline
~~{\tt source\_moved}          ~~& ~~{\tt source} with one big folder (a few megabits) renamed.~~\\\hline
~~{\tt firefox-13.0}           ~~& ~~The source archive of Mozilla Firefox 13.0.\\\hline
~~{\tt firefox-13.0.1}         ~~& ~~The source archive of Mozilla Firefox 13.0.1\\\hline
~~{\tt empty}                  ~~& ~~An empty folder.\\\hline
...
\hline
  ...
  \end{tabularx}
  \caption{Experimental results. Synchronization is performed \emph{from} Neil \emph{to} Oscar. RX and TX are
