...
\begin{compactitem}
  ...
  \item In Section~\ref{sec:related-work}, we compare \df with existing set
...
In this section, we compare \df with existing algorithms for set reconciliation.

\df can be seen as a independently found number-theoretical variant of the polynomial-based algorithm of Minsky, Trachtenberg and Zippel in~\cite{Mins1} (named MTZ in the sequel).
\df and MTZ have roughly the same transmission complexity, if we omit the fact that our algorithm only deals with fixed-size prime numbers, whereas their algorithm deal with any fixed-size bitstring.
We discuss the computational complexity of these two algorithms in~\ref{sec:opt-MTZ}: we show that they are quite close and that the optimizations we introduce can also be used with \cite{Mins1} to improve substantially its time efficiency.
