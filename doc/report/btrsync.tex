...
Let us write $P_i = p_1 \dots p_i$ and $ $.
After receving the redundancies $c_1,\dots,c_k$ corresponding to $p_1,\dots,p_k$, Neil has as many information as if Oscar had transmitted a redundancy for $c$ corresponding to the modulo $P_k$.
Therefore, the number $\lambda$ of rounds used is the minimum number $\lambda$ such that $T_k \ge t_0$.
...
In this section, we first present some variants of the protocol we described above, and then we analyse the complexity of all these variants.
...
TODO: goal $\longrightarrow$ we do not require to use storage... + last variant remove CRT and constant factor gains...
...
TODO: goal remove most complex operation !
...
\begin{table}
  ...
  \begin{tabularx}{\textwidth}{lXp{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}}
\toprule
{\bf \hfill Entity \hfill \null} & {\bf \hfill Computation \hfill \null} &  \multicolumn{4}{c}{{\bf Complexity expressed in $\Oapp$ of}} \\
\cmidrule(l){3-6}
& & \multicolumn{2}{c}{{\bf Basic algo.}} & \multicolumn{2}{c}{{\bf Opt. algo.}} \\
& & hash to prime & not hash to primes & hash to prime & not hash to primes \\
\midrule
Both  & computation of $h_i$ and $h'_i$ & $n u^2 \mu(u)$  &  \\
      & \textit{for round $i$} &  &  \\
Both  & compute redundancies $c_i$ and $c'_i$ & $n \cdot \mu(ut)$ &  & $\frac{n}{t} \cdot \mu(ut)$  \\
TODO \\
\bottomrule
  \end{tabularx}
...
\section{From Efficient Set Reconciliation to Efficient Files Synchronization}

TODO: expliquer que l'on utilise d'abord le hashé du path+content, puis ensuite on fait le move reconciliation sur le hashé des content (avant d'envoyer tous les fichiers) et enfin, on fait une copie intelligente des fichiers restants (via rsync)...

\subsection{TODO}
TODO: rewrite this as explained in the previous TODO !!!!
