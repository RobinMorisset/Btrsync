...
The complexity of a naive algorithm to perform $\tt HashPrime$ is $u^2 \mu(u)$, as for algorithms presented in~\cite{C:HohWat09,PKC:AbdBenPoi13}.
...
\full{With the optimization of Section~\ref{alg:div-prod-tree}}{With the product tree optimizations of Section~\ref{sec:algo-opt-prod-trees}}, it costs $\Oapp(\frac{n}{T_k} \mu(nuT_k))$, if we consider that the multiplication of a degree $T_k$ polynomial over $\mathbb{F}_q$ is $\mu(uT_k)$.
