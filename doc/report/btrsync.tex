% TODO: spellcheck
% TODO: correct figures, put algo 1/2 together, put table title above tables, put proofs in appendix, ...
...
\titlerunning{From Rational Number Reconstruction to Set Reconciliation and File Synchronization}
\authorrunning{A. Amarilli, F. Ben Hamouda, F. Bourse, R. Morisset, D. Naccache, P. Rauzy}
...
As explained in the introduction, various algorithms for set reconciliations have already been proposed in~\cite{byers,Whats,PSRec,Mins1}.
Let us compare \df with all these algorithms.
\todo{TODO (Fabrice): this may not be the good place !! and it is important to ntoe that we go further simple set reconciliation}


Firstly, \df can be seen as a (independently found) number theoretical variant of the polynomial based algorithm of Minsky, Trachtenberg and Zippel in~\cite{Mins1} (named MTZ in the sequel).
Its transmission complexity is roughly the same, if we omit the fact that our algorithm only deal with fixed-size prime numbers, whereas theirs deal with any fixed-size bitstring.
We discuss the computational complexity of these two algorithms in~\ref{sec:opt-MTZ} and show that they are quite close and that the optimizations we introduce can also be used with \cite{Mins1} to improve substantially its time efficiency.
\todo{TODO (Fabrice): another point is that we use a global hash to check termination, whereas MTZ uses another method which is maybe less optimal ! But this is quite trivial...

TODO: introduce $\Oapp$}

Secondly, the algorithm, which we call PART-MTZ, uses MTZ as a blackbox, and which performs set reconciliation using at least about $24e$ times more bandwith (with a partition in $2$ parts), but substantially improves the computational complexity of MTZ.
Since this algorithm is actually not specific to MTZ, it can also be used with \df.
Let us call PART-\df this new algorithm.
PART-\df and PART-MTZ have roughly the same transmission complexity.
We compare their computational complexity in~\ref{sec:opt-MTZ}.

Finally, let us compare \df and MTZ with the algorithms described in~\cite{Whats} and~\cite{byers}, which we call DD and ART.
In~\cite{Whats}, the authors compared MTZ, DD and ART.
Due to the fact MTZ and \df have roughly the same transmission complexity and computational complexity, this comparison is still valid when we replace MTZ by \df.
It shows that MTZ and \df use less bandwidth than DD (by a constant factor), and that it uses a lot less bandwidth than ART, when the number of differences $T$ is small enough (less than 10\% of the size of $\Set \cup \Set'$).
Concerning the computational complexity, it shows that MTZ and \df without any of the optimizations introduced in~\ref{sec:comp} are slower than DD.
The comparison of the time efficiency of PART-MTZ, PART-\df and optimized MTZ and \df (with optimizations in~\ref{sec:comp}) seems to be promising and is left as future work.
...
After receiving the redundancies $c_1,\dots,c_k$ corresponding to $p_1,\dots,p_k$, Neil has as much information as if Oscar had transmitted a redundancy $C_k$ corresponding to the modulo $P_k$, and can compute $S_k = C'_k / C_k$ from $s_k = c'_k/c_k$ and $S_{k-1}$ using the Chinese remainder theorem (CRT)\todo{(TODO ref ?)}: 
\begin{align*}
...
\todo{TODO: check this table and improve it ?}
...
when benchmarking \rsync are given in \full{Table~\ref{tab:rsyncopt}}{\apref{}}. Experiments
...
~~{\tt syn\_shuf}    ~~&~~{\tt syn} with: 10 deleted files, 10 renamed files 10 modified files \\
...
\begin{table}[t]
  ...
  \label{tab:rsyncopt}
  \begin{tabular}{p{\dimexpr 0.3\linewidth-2\tabcolsep} p{\dimexpr
    0.7\linewidth-2\tabcolsep}}
    \toprule
